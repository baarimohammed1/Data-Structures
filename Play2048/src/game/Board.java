...
public class Board {
        ...
        for (int row = 0; row < gameBoard.length; row++) {
            for (int column = 0; column < gameBoard[row].length; column++) {
                if (gameBoard[row][column] == 0) {
                    openSpaces.add(new BoardSpot(row, column));
                }
            }
        }
    }
    ...
    public void addRandomTile() {
            ...
            int value = StdRandom.uniform() < 0.1 ? 4 : 2;
    ...
    public void swipeLeft() {
            for (int i = 0; i < gameBoard.length; i++) {
                ...
                for (int j = 0; j < gameBoard[i].length; j++) {
                    if (gameBoard[i][j] != 0) {
                        if (j != currentPointer) {
                            gameBoard[i][currentPointer] = gameBoard[i][j];
                            gameBoard[i][j] = 0;
                        }
                        currentPointer++;
                    }
    ...
    public void mergeLeft() {
        for (int row = 0; row < gameBoard.length; row++) {
            for (int col = 0; col < gameBoard[row].length - 1; col++) {
                ...
                int next = gameBoard[row][col + 1];
                if (current == next) {
                    gameBoard[row][col] =  current *2;
