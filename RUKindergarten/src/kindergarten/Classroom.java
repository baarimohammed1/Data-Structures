...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        for (int i = 0; i < number; i++) {
            ...
            Student nStudent = new Student(firstName, lastName, height);
         
            SNode newNode = new SNode(nStudent,studentsInLine);
    ...
    public void setupSeats(String seatingChart) {
        // Step 1: Open the file using StdIn.setFile(seatingChart);
        StdIn.setFile(seatingChart);
    
        // Step 2: Read the seating chart input file
        int r = StdIn.readInt();
        int c = StdIn.readInt();
        ...
        for (int i = 0; i < r; i++) {
            for (int j = 0; j < c; j++) {
                ...
                seatingLocation[i][j] = seatAvailable;
            }
        }
    }
            ...
            for (int j = 0; j < seatingLocation[i].length; j++) {
                if(seatingLocation[i][j] == true){
                    if(musicalChairs != null){
                        ...
                        SNode temp = musicalChairs.getNext().getNext();
    ...
    public void insertMusicalChairs() {
        // Traverse the studentsSitting array row-wise, then column-wise
        for (int i = 0; i < studentsSitting.length; i++) {
            for (int j = 0; j < studentsSitting[i].length; j++) {
                // Get the student at the current position
                Student student = studentsSitting[i][j];
                // If the position is not empty, add the student to the musicalChairs linked list
                if (student != null) {
                    // If the musicalChairs list is empty, create a new node and make it the last node
                    if (musicalChairs == null) {
                        musicalChairs.next = musicalChairs;
    ...
    public void moveStudentFromChairsToLine(int size) {
     ...
     // Delete the selected student from musicalChairs CLL
